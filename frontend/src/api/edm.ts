...
export const edmApi = {
  ...
  createCampaign: async (campaign: EdmCampaign) => {
    const { data } = await axios.post<any>(`${API_BASE}`, campaign)
    return data.data as EdmCampaign
  },

  // 更新 EDM 活動
  updateCampaign: async (id: number, campaign: Partial<EdmCampaign>) => {
    const { data } = await axios.put<any>(`${API_BASE}/${id}`, campaign)
    return data.data as EdmCampaign
  },
  ...
  sendCampaign: async (id: number) => {
    const { data } = await axios.post<any>(`${API_BASE}/${id}/send`)
    return data.success
  },
  ...
  // 獲取 EDM 統計資訊
  getStatistics: async () => {
    const { data } = await axios.get<any>(`${API_BASE}/statistics`)
